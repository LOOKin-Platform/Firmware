 ...
 * @return N/A.
 */
void WiFi_t::addDNSServer(std::string ip) {
	ip_addr_t dnsserver;
 ...
 * @return N/A.
 */
void WiFi_t::connectAP(std::string ssid, std::string password){
	::tcpip_adapter_init();
 ...
 */
struct in_addr WiFi_t::getHostByName(std::string hostName) {
 ...
 */
std::vector<WiFiAPRecord> WiFi_t::scan() {
	::tcpip_adapter_init();
	ESP_ERROR_CHECK(esp_event_loop_init(wifiEventHandler->getEventHandler(), wifiEventHandler));
	wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
	ESP_ERROR_CHECK( esp_wifi_init(&cfg) );
 ...
 * @return N/A.
 */
void WiFi_t::startAP(std::string ssid, std::string password) {
	::nvs_flash_init();
	::tcpip_adapter_init();
	ESP_ERROR_CHECK(esp_event_loop_init(wifiEventHandler->getEventHandler(), wifiEventHandler));
	wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
 ...
 * @return N/A.
 */
void WiFi_t::setIPInfo(std::string ip, std::string gw, std::string netmask) {
	this->ip = ip;
	this->gw = gw;
