...
void BluetoothPeriodicHandler::Pool() {
	...
	if (Time::Uptime() < 5 && BluetoothCountdown == 0) {
		BluetoothCountdown = Settings.Bluetooth.AliveIntervals[Settings.Bluetooth.IntervalID].first;
	}
	...
	if (!Time::IsUptime(Time::Unixtime()) && IsTimeSynced == false) {
		...
		BluetoothCountdown = Settings.Bluetooth.AliveIntervals[Settings.Bluetooth.IntervalID].first -
							Expired % ((Settings.Bluetooth.AliveIntervals[Settings.Bluetooth.IntervalID].first
							+ Settings.Bluetooth.AliveIntervals[Settings.Bluetooth.IntervalID].second) / 1000);
	}
	...
	if (BluetoothCountdown <=0) {
		if (!BLEServer.IsRunning()) {
			...
			BluetoothCountdown = Settings.Bluetooth.AliveIntervals[Settings.Bluetooth.IntervalID].first
							+ Settings.Bluetooth.AliveIntervals[Settings.Bluetooth.IntervalID].second;
			BLEServer.StartAdvertising("0" +
					Converter::ToHexString(Settings.Bluetooth.IntervalID,1) +
	...
	if (BLEServer.IsRunning()) {
		...
		if (Time::UptimeU() > BluetoothStartedTime + Settings.Bluetooth.AliveIntervals[Settings.Bluetooth.IntervalID].second * 1000) {
