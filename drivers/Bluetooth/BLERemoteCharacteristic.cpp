...
static const char* tag = "BLERemoteCharacteristic";   // The logging tag for this class.
...
BLERemoteCharacteristic::BLERemoteCharacteristic(
	...
	ESP_LOGD(tag, ">> BLERemoteCharacteristic: handle: %d 0x%d, uuid: %s", handle, handle, uuid.ToString().c_str());
	m_handle         = handle;
	m_uuid           = uuid;
	m_charProp       = charProp;
	...
	ESP_LOGD(tag, "<< BLERemoteCharacteristic");
	...
	switch(event) {
		...
		case ESP_GATTC_NOTIFY_EVT: {
			...
			if (m_notifyCallback != nullptr) {
				ESP_LOGD(tag, "Invoking callback for notification on characteristic %s", ToString().c_str());
...
void BLERemoteCharacteristic::RetrieveDescriptors() {
	ESP_LOGD(tag, ">> retrieveDescriptors() for characteristic: %s", GetUUID().ToString().c_str());
	...
	while(true) {
		...
		if (status != ESP_GATT_OK) {
			ESP_LOGE(tag, "esp_ble_gattc_get_all_descr: %s", BLEUtils::GattStatusToString(status).c_str());
			break;
		}

		if (count == 0) break;

		ESP_LOGD(tag, "Found a descriptor: Handle: %d, UUID: %s", result.handle, BLEUUID(result.uuid).ToString().c_str());
	...
	ESP_LOGD(tag, "<< retrieveDescriptors(): Found %d descriptors.", offset);
} // getDescriptors
...
uint16_t BLERemoteCharacteristic::GetHandle() {
	//ESP_LOGD(tag, ">> getHandle: Characteristic: %s", getUUID().ToString().c_str());
	//ESP_LOGD(tag, "<< getHandle: %d 0x%.2x", m_handle, m_handle);
	return m_handle;
} // getHandle
...
BLERemoteDescriptor* BLERemoteCharacteristic::GetDescriptor(BLEUUID uuid) {
	ESP_LOGD(tag, ">> getDescriptor: uuid: %s", uuid.ToString().c_str());
	std::string v = uuid.ToString();
	for (auto &myPair : m_descriptorMap) {
		if (myPair.first == v) {
			ESP_LOGD(tag, "<< getDescriptor: found");
			return myPair.second;
		}
	}
	ESP_LOGD(tag, "<< getDescriptor: Not found");
	return nullptr;
} // getDescriptor
...
std::string BLERemoteCharacteristic::ReadValue() {
	ESP_LOGD(tag, ">> readValue(): uuid: %s, handle: %d 0x%.2x", GetUUID().ToString().c_str(), GetHandle(), GetHandle());
...
void BLERemoteCharacteristic::RegisterForNotify(notify_callback notifyCallback, bool notifications) {
	ESP_LOGD(tag, ">> registerForNotify(): %s", ToString().c_str());
	...
	if (notifyCallback != nullptr) {   // If we have a callback function, then this is a registration.
		...
		if (errRc != ESP_OK) {
			ESP_LOGE(tag, "esp_ble_gattc_register_for_notify: rc=%d %s", errRc, GeneralUtils::ErrorToString(errRc));
		}
		...
		if (errRc != ESP_OK) {
			ESP_LOGE(tag, "esp_ble_gattc_unregister_for_notify: rc=%d %s", errRc, GeneralUtils::ErrorToString(errRc));
		}
...
bool BLERemoteCharacteristic::WriteValue(uint8_t* data, size_t length, bool response) {
	...
	ESP_LOGD(tag, ">> writeValue(), length: %d", length);
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(tag, "esp_ble_gattc_write_char: rc=%d %s", errRc, GeneralUtils::ErrorToString(errRc));
		return false;
	}
