...
static const char* tag = "GeneralUtils";
...
void GeneralUtils::DumpInfo() {
	...
	ESP_LOGV(tag, "--- dumpInfo ---");
	ESP_LOGV(tag, "Free heap: %d", freeHeap);
	ESP_LOGV(tag, "Chip Info: Model: %d, cores: %d, revision: %d", chipInfo.model, chipInfo.cores, chipInfo.revision);
	ESP_LOGV(tag, "ESP-IDF version: %s", esp_get_idf_version());
	ESP_LOGV(tag, "---");
} // dumpInfo
