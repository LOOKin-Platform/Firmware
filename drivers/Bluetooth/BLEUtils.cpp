...
const char* BLEUtils::advTypeToString(uint8_t advType) {
	switch(advType) {
		...
		default:
			ESP_LOGD(tag, " adv data type: 0x%x", advType);
			return "";
	} // End switch
} // advTypeToString
...
//			return "ESP_GATTC_GET_INCL_SRVC_EVT";
		...
		default:
			ESP_LOGW(tag, "Unknown GATT Client event type: %d", eventType);
			return "Unknown";
	}
...
void BLEUtils::dumpGapEvent(
	...
	ESP_LOGD(tag, "Received a GAP event: %s", gapEventToString(event));
	switch(event) {
		//
		...
		case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT: {
			ESP_LOGD(tag, "[status: %d]",	param->adv_data_cmpl.status);
			break;
		} // ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT
		...
		case ESP_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT: {
			ESP_LOGD(tag, "[status: %d]",	param->adv_data_raw_cmpl.status);
			break;
		} // ESP_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT
		...
		case ESP_GAP_BLE_ADV_START_COMPLETE_EVT: {
			ESP_LOGD(tag, "[status: %d]",	param->adv_start_cmpl.status);
			break;
		} // ESP_GAP_BLE_ADV_START_COMPLETE_EVT
		...
		case ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT: {
			ESP_LOGD(tag, "[status: %d]",	param->adv_stop_cmpl.status);
			break;
		} // ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT
		...
		case ESP_GAP_BLE_CLEAR_BOND_DEV_COMPLETE_EVT: {
			ESP_LOGD(tag, "[status: %d]",	param->clear_bond_dev_cmpl.status);
			break;
		} // ESP_GAP_BLE_CLEAR_BOND_DEV_COMPLETE_EVT
		...
		case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
			ESP_LOGD(tag, "[status: %d]",	param->scan_param_cmpl.status);
			break;
		} // ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT
		...
		case ESP_GAP_BLE_SCAN_RESULT_EVT: {
			switch(param->scan_rst.search_evt) {
				...
				default: {
					ESP_LOGD(tag, "search_evt: %s",searchEventTypeToString(param->scan_rst.search_evt));
					break;
				}
			}
			break;
		...
		case ESP_GAP_BLE_SCAN_RSP_DATA_SET_COMPLETE_EVT: {
			ESP_LOGD(tag, "[status: %d]",	param->scan_rsp_data_cmpl.status);
			break;
		} // ESP_GAP_BLE_SCAN_RSP_DATA_SET_COMPLETE_EVT
		...
		case ESP_GAP_BLE_SCAN_RSP_DATA_RAW_SET_COMPLETE_EVT: {
			ESP_LOGD(tag, "[status: %d]",	param->scan_rsp_data_raw_cmpl.status);
			break;
		} // ESP_GAP_BLE_SCAN_RSP_DATA_RAW_SET_COMPLETE_EVT
		...
		case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT: {
			ESP_LOGD(tag, "[status: %d]", param->scan_stop_cmpl.status);
			break;
		} // ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT
		...
		default: {
			ESP_LOGD(tag, "*** dumpGapEvent: Logger not coded ***");
			break;
		} // default
	} // switch
} // dumpGapEvent
...
void BLEUtils::dumpGattClientEvent(
	...
	ESP_LOGD(tag, "GATT Event: %s", BLEUtils::gattClientEventTypeToString(event).c_str());
	switch(event) {
		//
		// ESP_GATTC_CLOSE_EVT
		//
		...
		//
		case ESP_GATTC_CLOSE_EVT: {
			ESP_LOGD(tag, "[status: %s, reason:%s, conn_id: %d]",
		...
		case ESP_GATTC_GET_CHAR_EVT: {
			...
			if (evtParam->get_char.status == ESP_GATT_OK) {
				...
				ESP_LOGD(tag, "[status: %s, conn_id: %d, srvc_id: %s, char_id: %s [description: %s]\nchar_prop: %s]",
				...
				ESP_LOGD(tag, "[status: %s, conn_id: %d, srvc_id: %s]",
					BLEUtils::gattStatusToString(evtParam->get_char.status).c_str(),
					evtParam->get_char.conn_id,
					BLEUtils::gattServiceIdToString(evtParam->get_char.srvc_id).c_str()
				);
		...
		*/
		...
		// uint16_t           value_len
		case ESP_GATTC_READ_CHAR_EVT: {
			ESP_LOGD(tag, "[status: %s, conn_id: %d, handle: %d 0x%.2x, value_len: %d]",
			...
			if (evtParam->read.status == ESP_GATT_OK) {
				...
				ESP_LOGD(tag, "value: %s \"%s\"", pHexData, BLEUtils::buildPrintData(evtParam->read.value, evtParam->read.value_len).c_str());
				free(pHexData);
				*/
			}
			break;
		...
		//
		case ESP_GATTC_REG_EVT: {
			ESP_LOGD(tag, "[status: %s, app_id: 0x%x]",
		...
		//
		case ESP_GATTC_SEARCH_CMPL_EVT: {
			ESP_LOGD(tag, "[status: %s, conn_id: %d]",
		...
		//
		case ESP_GATTC_SEARCH_RES_EVT: {
			ESP_LOGD(tag, "[conn_id: %d, start_handle: %d 0x%.2x, end_handle: %d 0x%.2x, srvc_id: %s",
		...
		//
		case ESP_GATTC_WRITE_CHAR_EVT: {
			ESP_LOGD(tag, "[status: %s, conn_id: %d, handle: %d 0x%.2x]",
...
void BLEUtils::dumpGattServerEvent(
	...
	ESP_LOGD(tag, "GATT ServerEvent: %s", BLEUtils::gattServerEventTypeToString(event).c_str());
	switch(event) {
		...
		//
		case ESP_GATTS_CONF_EVT: {
			ESP_LOGD(tag, "[status: %s, conn_id: 0x%.2x]",
		...
		case ESP_GATTS_CONGEST_EVT: {
			ESP_LOGD(tag, "[conn_id: %d, congested: %d]",
		...
		case ESP_GATTS_RESPONSE_EVT: {
			ESP_LOGD(tag, "[status: %s, handle: 0x%.2x]",
		...
		//
		case ESP_GATTS_START_EVT: {
			ESP_LOGD(tag, "[status: %s, service_handle: 0x%.2x]",
...
const char* BLEUtils::eventTypeToString(esp_ble_evt_type_t eventType) {
	switch(eventType) {
		...
		default:
			ESP_LOGD(tag, "Unknown esp_ble_evt_type_t: %d (0x%.2x)", eventType, eventType);
			return "*** Unknown ***";
	}
} // eventTypeToString
...
const char* BLEUtils::gapEventToString(uint32_t eventType) {
	switch(eventType) {
		...
		default:
			ESP_LOGD(tag, "gapEventToString: Unknown event type %d 0x%.2x", eventType, eventType);
...
const char* BLEUtils::searchEventTypeToString(esp_gap_search_evt_t searchEvt) {
	switch(searchEvt) {
		...
		default:
			ESP_LOGD(tag, "Unknown event type: 0x%x", searchEvt);
			return "Unknown event type";
	}
