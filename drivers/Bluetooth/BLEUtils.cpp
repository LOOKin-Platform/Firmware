...
static char tag[] = "BLEUtils";  // Tag for logging.

/*
static std::map<std::string, BLEClientGeneric*> g_addressMap;
static std::map<uint16_t, BLEClientGeneric*> g_connIdMap;
*/
...
const char* BLEUtils::AdvTypeToString(uint8_t advType) {
	switch (advType) {
		...
		default:
			ESP_LOGV(tag, " adv data type: 0x%x", advType);
			return "";
	} // End switch
} // advTypeToString
...
char* BLEUtils::BuildHexData(uint8_t* target, uint8_t* source, uint8_t length) {
	...
	if (target == nullptr) {
		...
		if (target == nullptr) {
			ESP_LOGE(tag, "buildHexData: malloc failed");
			return nullptr;
		}
	}
...
//			return "ESP_GATTC_GET_INCL_SRVC_EVT";
		...
		default:
			ESP_LOGV(tag, "Unknown GATT Client event type: %d", eventType);
			return "Unknown";
	}
...
void BLEUtils::DumpGapEvent(
	...
	ESP_LOGV(tag, "Received a GAP event: %s", GapEventToString(event));
	switch (event) {
#if CONFIG_LOG_DEFAULT_LEVEL > 4
		...
		case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT: {
			ESP_LOGV(tag, "[status: %d]", param->adv_data_cmpl.status);
			break;
		} // ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT
		...
		case ESP_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT: {
			ESP_LOGV(tag, "[status: %d]", param->adv_data_raw_cmpl.status);
			break;
		} // ESP_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT
		...
		case ESP_GAP_BLE_ADV_START_COMPLETE_EVT: {
			ESP_LOGV(tag, "[status: %d]", param->adv_start_cmpl.status);
			break;
		} // ESP_GAP_BLE_ADV_START_COMPLETE_EVT
		...
		case ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT: {
			ESP_LOGV(tag, "[status: %d]", param->adv_stop_cmpl.status);
			break;
		} // ESP_GAP_BLE_ADV_STOP_COMPLETE_EVT
		...
		case ESP_GAP_BLE_AUTH_CMPL_EVT: {
			ESP_LOGV(tag, "[bd_addr: %s, key_present: %d, key: ***, key_type: %d, success: %d, fail_reason: %d, addr_type: ***, dev_type: %s]",
		...
		case ESP_GAP_BLE_CLEAR_BOND_DEV_COMPLETE_EVT: {
			ESP_LOGV(tag, "[status: %d]", param->clear_bond_dev_cmpl.status);
			break;
		} // ESP_GAP_BLE_CLEAR_BOND_DEV_COMPLETE_EVT
		...
		case ESP_GAP_BLE_NC_REQ_EVT: {
			ESP_LOGV(tag, "[bd_addr: %s, passkey: %d]",
		...
		case ESP_GAP_BLE_READ_RSSI_COMPLETE_EVT: {
			ESP_LOGV(tag, "[status: %d, rssi: %d, remote_addr: %s]",
		...
		case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
			ESP_LOGV(tag, "[status: %d]", param->scan_param_cmpl.status);
			break;
		} // ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT
		...
		case ESP_GAP_BLE_SCAN_RESULT_EVT: {
			switch (param->scan_rst.search_evt) {
				case ESP_GAP_SEARCH_INQ_RES_EVT: {
					ESP_LOGV(tag, "search_evt: %s, bda: %s, dev_type: %s, ble_addr_type: %s, ble_evt_type: %s, rssi: %d, ble_adv: ??, flag: %d (%s), num_resps: %d, adv_data_len: %d, scan_rsp_len: %d",
				...
				default: {
					ESP_LOGV(tag, "search_evt: %s",searchEventTypeToString(param->scan_rst.search_evt));
					break;
				}
			}
			break;
		...
		case ESP_GAP_BLE_SCAN_RSP_DATA_SET_COMPLETE_EVT: {
			ESP_LOGV(tag, "[status: %d]", param->scan_rsp_data_cmpl.status);
			break;
		} // ESP_GAP_BLE_SCAN_RSP_DATA_SET_COMPLETE_EVT

		// ESP_GAP_BLE_SCAN_RSP_DATA_RAW_SET_COMPLETE_EVT
		case ESP_GAP_BLE_SCAN_RSP_DATA_RAW_SET_COMPLETE_EVT: {
			ESP_LOGV(tag, "[status: %d]", param->scan_rsp_data_raw_cmpl.status);
			break;
		} // ESP_GAP_BLE_SCAN_RSP_DATA_RAW_SET_COMPLETE_EVT
		...
		case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT: {
			ESP_LOGV(tag, "[status: %d]", param->scan_start_cmpl.status);
			break;
		} // ESP_GAP_BLE_SCAN_START_COMPLETE_EVT
		...
		case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT: {
			ESP_LOGV(tag, "[status: %d]", param->scan_stop_cmpl.status);
			break;
		} // ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT
		...
		case ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT: {
			ESP_LOGV(tag, "[status: %d, bd_addr: %s, min_int: %d, max_int: %d, latency: %d, conn_int: %d, timeout: %d]",
		...
		case ESP_GAP_BLE_SEC_REQ_EVT: {
			ESP_LOGV(tag, "[bd_addr: %s]", BLEAddress(param->ble_security.ble_req.bd_addr).ToString().c_str());
			break;
		} // ESP_GAP_BLE_SEC_REQ_EVT
#endif
		default: {
			ESP_LOGV(tag, "*** dumpGapEvent: Logger not coded ***");
			break;
		} // default
	} // switch
} // dumpGapEvent
...
void BLEUtils::DumpGattClientEvent(
	...
	ESP_LOGV(tag, "GATT Event: %s", BLEUtils::GattClientEventTypeToString(event).c_str());
	switch (event) {
#if CONFIG_LOG_DEFAULT_LEVEL > 4
		// ESP_GATTC_CLOSE_EVT
		//
		...
		case ESP_GATTC_CLOSE_EVT: {
			ESP_LOGV(tag, "[status: %s, reason:%s, conn_id: %d]",
		...
		// - esp_bd_addr_t     remote_bda
		case ESP_GATTC_CONNECT_EVT: {
			ESP_LOGV(tag, "[conn_id: %d, remote_bda: %s]",
		...
		// - esp_bd_addr_t          remote_bda
		case ESP_GATTC_DISCONNECT_EVT: {
			ESP_LOGV(tag, "[reason: %s, conn_id: %d, remote_bda: %s]",
		...
		case ESP_GATTC_GET_CHAR_EVT: {
			...
			if (evtParam->get_char.status == ESP_GATT_OK) {
				...
				ESP_LOGV(tag, "[status: %s, conn_id: %d, srvc_id: %s, char_id: %s [description: %s]\nchar_prop: %s]",
				...
				ESP_LOGV(tag, "[status: %s, conn_id: %d, srvc_id: %s]",
					BLEUtils::gattStatusToString(evtParam->get_char.status).c_str(),
					evtParam->get_char.conn_id,
					BLEUtils::gattServiceIdToString(evtParam->get_char.srvc_id).c_str()
				);
		...
		*/
		...
		//
		case ESP_GATTC_NOTIFY_EVT: {
			ESP_LOGV(tag, "[conn_id: %d, remote_bda: %s, handle: %d 0x%.2x, value_len: %d, is_notify: %d]",
		...
		//
		case ESP_GATTC_OPEN_EVT: {
			ESP_LOGV(tag, "[status: %s, conn_id: %d, remote_bda: %s, mtu: %d]",
		...
		// uint16_t           value_len
		case ESP_GATTC_READ_CHAR_EVT: {
			ESP_LOGV(tag, "[status: %s, conn_id: %d, handle: %d 0x%.2x, value_len: %d]",
			...
			if (evtParam->read.status == ESP_GATT_OK) {
				...
				ESP_LOGV(tag, "value: %s \"%s\"", pHexData, BLEUtils::buildPrintData(evtParam->read.value, evtParam->read.value_len).c_str());
				free(pHexData);
				*/
			}
			break;
		...
		// - uint16_t          app_id
		case ESP_GATTC_REG_EVT: {
			ESP_LOGV(tag, "[status: %s, app_id: 0x%x]",
		...
		// - uint16_t          handle
		case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
			ESP_LOGV(tag, "[status: %s, handle: %d 0x%.2x]",
		...
		// - uint16_t          conn_id
		case ESP_GATTC_SEARCH_CMPL_EVT: {
			ESP_LOGV(tag, "[status: %s, conn_id: %d]",
		...
		case ESP_GATTC_SEARCH_RES_EVT: {
			ESP_LOGV(tag, "[conn_id: %d, start_handle: %d 0x%.2x, end_handle: %d 0x%.2x, srvc_id: %s",
		...
		// - uint16_t          offset
		case ESP_GATTC_WRITE_CHAR_EVT: {
			ESP_LOGV(tag, "[status: %s, conn_id: %d, handle: %d 0x%.2x, offset: %d]",
...
void BLEUtils::DumpGattServerEvent(
	...
	ESP_LOGV(tag, "GATT ServerEvent: %s", BLEUtils::GattServerEventTypeToString(event).c_str());
	switch (event) {
#if CONFIG_LOG_DEFAULT_LEVEL > 4

		case ESP_GATTS_ADD_CHAR_DESCR_EVT: {
			ESP_LOGV(tag, "[status: %s, attr_handle: %d 0x%.2x, service_handle: %d 0x%.2x, char_uuid: %s]",
		...
		case ESP_GATTS_ADD_CHAR_EVT: {
			if (evtParam->add_char.status == ESP_GATT_OK) {
				...
				ESP_LOGE(tag, "[status: %s, attr_handle: %d 0x%.2x, service_handle: %d 0x%.2x, char_uuid: %s]",
					gattStatusToString(evtParam->add_char.status).c_str(),
					evtParam->add_char.attr_handle,
					evtParam->add_char.attr_handle,
					evtParam->add_char.service_handle,
		...
		case ESP_GATTS_CONF_EVT: {
			ESP_LOGV(tag, "[status: %s, conn_id: 0x%.2x]",
		...
		case ESP_GATTS_CONGEST_EVT: {
			ESP_LOGV(tag, "[conn_id: %d, congested: %d]",
		...
		case ESP_GATTS_CREATE_EVT: {
			ESP_LOGV(tag, "[status: %s, service_handle: %d 0x%.2x, service_id: [%s]]",
		...
		case ESP_GATTS_EXEC_WRITE_EVT: {
			...
			ESP_LOGV(tag, "[conn_id: %d, trans_id: %d, bda: %s, exec_write_flag: 0x%.2x=%s]",
		...
		case ESP_GATTS_READ_EVT: {
			ESP_LOGV(tag, "[conn_id: %d, trans_id: %d, bda: %s, handle: 0x%.2x, is_long: %d, need_rsp:%d]",
					evtParam->read.conn_id,
		...
		case ESP_GATTS_RESPONSE_EVT: {
			ESP_LOGV(tag, "[status: %s, handle: 0x%.2x]",
		...
		case ESP_GATTS_REG_EVT: {
			ESP_LOGV(tag, "[status: %s, app_id: %d]",
		...
		case ESP_GATTS_START_EVT: {
			ESP_LOGV(tag, "[status: %s, service_handle: 0x%.2x]",
		...
		case ESP_GATTS_WRITE_EVT: {
			ESP_LOGV(tag, "[conn_id: %d, trans_id: %d, bda: %s, handle: 0x%.2x, offset: %d, need_rsp: %d, is_prep: %d, len: %d]",
					evtParam->write.conn_id,
			...
			ESP_LOGV(tag, "[Data: %s]", pHex);
			free(pHex);
			break;
		} // ESP_GATTS_WRITE_EVT
#endif
		default:
			ESP_LOGV(tag, "dumpGattServerEvent: *** NOT CODED ***");
			break;
		}
} // dumpGattServerEvent
...
const char* BLEUtils::EventTypeToString(esp_ble_evt_type_t eventType) {
	switch (eventType) {
		...
		default:
			ESP_LOGV(tag, "Unknown esp_ble_evt_type_t: %d (0x%.2x)", eventType, eventType);
			return "*** Unknown ***";
	}
} // eventTypeToString
...
const char* BLEUtils::GapEventToString(uint32_t eventType) {
	switch (eventType) {
		...
		default:
			ESP_LOGV(tag, "gapEventToString: Unknown event type %d 0x%.2x", eventType, eventType);
...
const char* BLEUtils::SearchEventTypeToString(esp_gap_search_evt_t searchEvt) {
	switch (searchEvt) {
		...
		default:
			ESP_LOGV(tag, "Unknown event type: 0x%x", searchEvt);
			return "Unknown event type";
	}
