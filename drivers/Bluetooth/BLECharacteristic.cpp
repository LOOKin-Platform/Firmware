...
static const char* tag = "BLECharacteristic";
...
void BLECharacteristic::AddDescriptor(BLEDescriptor* pDescriptor) {
	ESP_LOGD(tag, ">> addDescriptor(): Adding %s to %s", pDescriptor->ToString().c_str(), ToString().c_str());
	m_descriptorMap.SetByUUID(pDescriptor->GetUUID(), pDescriptor);
	ESP_LOGD(tag, "<< addDescriptor()");
} // addDescriptor
...
void BLECharacteristic::ExecuteCreate(BLEService* pService) {
	...
	if (m_handle != NULL_HANDLE) {
		ESP_LOGE(tag, "Characteristic already has a handle.");
		return;
	}

	m_pService = pService; // Save the service to which this characteristic belongs.

	ESP_LOGD(tag, "Registering characteristic (esp_ble_gatts_add_char): uuid: %s, service: %s",
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(tag, "<< esp_ble_gatts_add_char: rc=%d %s", errRc, GeneralUtils::ErrorToString(errRc));
		return;
	}
...
void BLECharacteristic::HandleGATTServerEvent(
	...
	ESP_LOGD(tag, ">> handleGATTServerEvent: %s", BLEUtils::GattServerEventTypeToString(event).c_str());
	...
	// ???
				...
				if (errRc != ESP_OK) {
					ESP_LOGE(tag, "esp_ble_gatts_send_response: rc=%d %s", errRc, GeneralUtils::ErrorToString(errRc));
				}
			}
			break;
		} // ESP_GATTS_EXEC_WRITE_EVT
...
// to send a response.  If we do, then we formulate a response and send it.
			if (param->write.handle == m_handle) {
				...
				ESP_LOGD(tag, " - Response to write event: New value: handle: %.2x, uuid: %s",
						GetHandle(), GetUUID().ToString().c_str());

				char* pHexData = BLEUtils::BuildHexData(nullptr, param->write.value, param->write.len);
				ESP_LOGD(tag, " - Data: length: %d, data: %s", param->write.len, pHexData);
				free(pHexData);
...
//
				...
				ESP_LOGD(tag, "mtu value: %d", maxOffset);
				if (param->read.need_rsp) {
					...
					ESP_LOGD(tag, " - Data: length=%d, data=%s, offset=%d", rsp.attr_value.len, pHexData, rsp.attr_value.offset);
					free(pHexData);
		...
		case ESP_GATTS_CONF_EVT: {
			// ESP_LOGD(tag, "m_handle = %d, conf->handle = %d", m_handle, param->conf.handle);
...
void BLECharacteristic::Indicate() {
	ESP_LOGD(tag, ">> indicate: length: %d", m_value.GetValue().length());
	Notify(false);
	ESP_LOGD(tag, "<< indicate");
} // indicate
...
void BLECharacteristic::Notify(bool is_notification) {
	ESP_LOGD(tag, ">> notify: length: %d", m_value.GetValue().length());
	...
	if (GetService()->GetServer()->GetConnectedCount() == 0) {
		ESP_LOGD(tag, "<< notify: No connected clients.");
		return;
	}
	...
	if(p2902 == nullptr){
		ESP_LOGE(tag, "Characteristic without 0x2902 descriptor");
		return;
	}
	if(is_notification) {
		if (p2902 != nullptr && !p2902->GetNotifications()) {
			ESP_LOGD(tag, "<< notifications disabled; ignoring");
			return;
		}
	}
	else{
		if (p2902 != nullptr && !p2902->GetIndications()) {
			ESP_LOGD(tag, "<< indications disabled; ignoring");
			return;
		}
	}
	for (auto &myPair : GetService()->GetServer()->GetPeerDevices(false)) {
		...
		if (m_value.GetValue().length() > _mtu - 3) {
			ESP_LOGW(tag, "- Truncating to %d bytes (maximum notify size)", _mtu - 3);
		}
		...
		if (errRc != ESP_OK) {
			ESP_LOGE(tag, "<< esp_ble_gatts_send_ %s: rc=%d %s",is_notification?"notify":"indicate", errRc, GeneralUtils::ErrorToString(errRc));
			m_semaphoreConfEvt.Give();
			return;
		}
		if(!is_notification)
...
void BLECharacteristic::SetBroadcastProperty(bool value) {
	//ESP_LOGD(tag, "setBroadcastProperty(%d)", value);
	if (value) {
...
void BLECharacteristic::SetCallbacks(BLECharacteristicCallbacks* pCallbacks) {
	ESP_LOGD(tag, ">> setCallbacks: 0x%x", (uint32_t)pCallbacks);
...
void BLECharacteristic::SetHandle(uint16_t handle) {
	ESP_LOGD(tag, ">> setHandle: handle=0x%.2x, characteristic uuid=%s", handle, GetUUID().ToString().c_str());
	m_handle = handle;
...
void BLECharacteristic::SetIndicateProperty(bool value) {
	//ESP_LOGD(tag, "setIndicateProperty(%d)", value);
	if (value) {
...
void BLECharacteristic::SetNotifyProperty(bool value) {
	//ESP_LOGD(tag, "setNotifyProperty(%d)", value);
	if (value) {
...
void BLECharacteristic::SetReadProperty(bool value) {
	//ESP_LOGD(tag, "setReadProperty(%d)", value);
	if (value) {
...
void BLECharacteristic::SetValue(uint8_t* data, size_t length) {
	...
	ESP_LOGD(tag, ">> setValue: length=%d, data=%s, characteristic UUID=%s", length, pHex, GetUUID().ToString().c_str());
	free(pHex);
...
void BLECharacteristic::SetWriteNoResponseProperty(bool value) {
	//ESP_LOGD(tag, "setWriteNoResponseProperty(%d)", value);
	if (value)
...
void BLECharacteristic::SetWriteProperty(bool value) {
	//ESP_LOGD(tag, "setWriteProperty(%d)", value);
	if (value) {
...
string BLECharacteristic::ToString() {
	...
	stringstream << "UUID: " << m_bleUUID.ToString() + ", handle: 0x" << setw(2) << m_handle;
	stringstream << " " <<
