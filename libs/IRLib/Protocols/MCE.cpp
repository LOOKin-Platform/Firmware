...
class MCE : public IRProto {
	private:
		...
		uint8_t Parse8bits(vector<int32_t> &Data, uint8_t Start)
		{
			...
			for (int i=0; i < 8; i++)
				Bits[8-i] = isOneBit(Data[Start + i*2], Data[Start + i*2+1]);
	...
	public:
		...
		bool IsProtocol(vector<int32_t> &RawData, uint16_t Start, uint16_t Length) override {
			...
			if !(TestValue(RawData[Start], 2664) && TestValue(RawData[Start + 1], -TrailerBitHalfLen) &&
				TestValue(RawData[Start + 2], NormalBitHalfLen) && TestValue(RawData[Start + 3], -NormalBitHalfLen))
				return false;
