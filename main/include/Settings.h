...
class Settings_t {
	public:
		FirmwareVersion 					Firmware 		= FirmwareVersion(3, 0, 89);
		...
		struct {
			...
			const string 					SyncTime		= "pool.ntp.org";
		...
		struct {
			struct {
				const uint16_t				Threshold 		= 65000; 	// Максимальное значени интервала, которое принимается в обработке
